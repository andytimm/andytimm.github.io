...
but it's worth reemphasizing that we're probably trying to represent something complex,
...
like it gets time expensive fast for the user quickly. If I'm going to have
...
look like our posterior. By having that transforming component be a neural network,
...
arbitrarily complex distributions that remain valid probability distributions.

![Image Credit to [Simon Boehm](https://siboehm.com/articles/19/normalizing-flow-network) here](images/normalizing-flow.png)

If we have a random variable $z$, with distribution $q(z)$, some function $f$ with an inverse
$f^{-1} = g, g \circ f(z) = z$, then the distribution of the result of
one iteration of z through, $q^\prime(z)$ is:

$$
q\prime(z) = q(z) \lvert det \frac{\partial f^{-1}}{\partial z^\prime} \rvert = q(z) \lvert \frac{\partial f}{\partial z} \rvert^{-1}
$$
...
we've got a chain of transformations (eg $f_K(...(f_2f_1(z))$:

$$
z_K = f(z) \circ ... \circ f_2 \circ f_1(z_0)
$$

then the resulting density $q_K(z)$ looks like:

$$
ln q_K (z_K) = lnq_0(z_0) - \sum \limits_{K = 1}\limits^{K} ln  \lvert \frac{\partial f_k}{\partial z_{k-1}} \rvert^{-1}
$$

This is a bit involved, so let's back up for some intuition. With a setup
like this, we can sample from something super simple, and plug and chug
through the above calculations to get our samples from the transformed
distribution which we can model to fix our posterior target well.

That said, We want to be able
to apply a bunch of transformations like this efficiently so that we can iterate
scalably. When is that
true? There are
two main operations here we need to care about the efficiency of: we need 
$f$ to be easily invertible, and we need to be able to calculate the determinant
of it's Jacobian easily as well. 
