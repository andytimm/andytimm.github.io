...
hard to fit with a normal variational family, but it's pretty easy to define in PyTorch as well:
...
Here's a gif of what that looks like over the course of training. With just a single
layer of planar  of course, this isn't expressive enough to capture the full density, but we can see why
...
there are just flows developed since the original normalizing flows paper
...
of a *coupling flow*, where if the flow is $D$-dimensional, the first half of the dimensions $1:d$
...
Where s and t are scaling and transformation functions from $\mathbb{R}^d \mathbb{R}^{D-d}$,
and $\odot$ is the Hadamard (element-wise) product. Visually, at each layer:

![Figure credit due to [Eric Jang](https://blog.evjang.com/2018/01/nf2.html); he prefers $\alpha$ and $\mu$ instead of $s$ and $t$](images/real_nvp_illustration.png){fig-alt="transforming half of the dimensions as a function of the other half"}

This has a lot of really appealing properties. First, this
