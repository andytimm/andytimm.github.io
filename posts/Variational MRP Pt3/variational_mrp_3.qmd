...
$$
\frac{1}{2}log(1 + D_{\chi^2}(p(z|x)|q(z;\lambda))) = -logp(x) + \frac{1}{2}log    
$$
...
$$
\mathscr{L}_{\chi^2}(\lambda) = \mathbb{E}_{q(z;\lambda)}[(\frac{p(z|x)}{q(z;\lambda)})^2]
$$
Since this new divergence is non-negative as well, this is a upper bound of the model evidence. This is thus the $\chi$ upper bound (CUBO).
