...
# Responding to some common reasons to use MaxDiff

I had originally planned to treat this section as an attempt to [Steelman](https://en.wikipedia.org/wiki/Straw_man#Steelmanning) Maxdiff, but I'm a little hesitant to call what I do in this section that. I don't really find the best arguments I know of
for MaxDiff particularly compelling. In this section of the post, I'll try to respond to
a non-comprehensive list of reasons to use MaxDiff I've read or heard over time.

## The advantages of the Sawtooth suite

As this post has argued, I don't agree with the core MaxDiff modeling implementation
that [Sawtooth leverages](https://sawtoothsoftware.com/resources/technical-papers/maxdiff-technical-paper),
and I think they inadequately defend it given the psychological and mathematical difficulties it represents.
Still, their product suite offers quite a lot for the discrete choice modeler:

1. A Gibbs Sampler implementation of their Bayesian models, which is significantly
faster for equivalent data than models like mine in Stan.
2. A very cool set of  tools ([MaxDiff Designer](https://sawtoothsoftware.com/help/discover2/survey-elements/maxdiff/design-settings)) for designing
MaxDiff question sets that are quite statistically efficient.
3. Genuinely good default choices of priors and other small details that are hard to get right;
more generally, a tool that's been widely used for years, and so is unlikely to be unreliable.
4. Most importantly, tools for deploying MaxDiff experiments without a lot of data wrangling, formating
hassles, and other naggles that make doing this yourself slow and annoying.


These are all great things, especially the last one. Just in the writing of this
post I spent a bunch of time debugging annoying indexing problems, debating how much
to try and optimize the design efficiency, and waiting for this
to all run again to proofread.

Putting aside convenience though, let's talk about this at the level of what's likely to produce correct estimates.
On that level, my intuition is "how to build the basic likelihood" is a pretty crucial
feature, one to get right before a lot of those other optimizations can differentiate on correctness.
So I'd claim: if you're going to claim on accuracy grounds you should use Sawtooth,
you should really pair that with a plausible story about why MaxDiff is at
least not significantly worse than other models for your data.

# What if I find symmetry of best/worsts more feasible for my data?

Let's next go through some ways MaxDiff might be understood as "fine for my data".

One bit of intuition
I once heard from someone in marketing was that they actually did think symmetrical preferences
were pretty plausible for their type of product. A way to frame this more mathematically would be "what if I think the true errors
ARE normal?"

That's a testable hypothesis via simulation, so let's test it.

<details>

<summary>**An example with normal errors**</summary>
...
ranked_options <- indexes %>% 
  ...
  mutate(
    fixed_utility = as.numeric(X[row,] %*% as.numeric(beta_i[first(individual),])),
    plus_normal_error = fixed_utility + rnorm(n(),0,3), # NOTE: normal errors!
    true_rank = rank(-plus_normal_error),
    true_order = order(true_rank),
    observed_order = case_when(
      true_order == J ~ J,  # Worst choice
      true_order == 1 ~ 1,  # Best choice
...
rmse_comparison_normal <- data.frame(
  Model = c("MaxDiff", "ROL with ties", "Efron Approximation", "ROL (no ties)"),
  RMSE = c(
    calculate_rmse(results_maxdiff),
    calculate_rmse(results_rol_ties),
...
fourway_comparison_plot_normal <- ggplot(all_results, aes(x = True_value_normalized, y = median, color = Dataset)) +
  geom_linerange(aes(ymin = lower, ymax = upper), alpha = 0.3) +
  geom_point(alpha = 0.5) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  labs(x = "Normalized True Utility",
...
```{R}
print(fourway_comparison_plot_normal)
print(rmse_comparison_normal)
```

As you can see, the error structure being normal, and thus encoding a notion of symmetrical preferences doesn't immediately lead
to MaxDiff being comparable to other models or better. Of course, this is only
1 possible way to cook up the simulation study- perhaps something in the simulation of utilities should change too.
So my refrain for situations like
this will be: if you want to defend MaxDiff in situations like these, come up with
a simulation structure where MaxDiff is less harmful, or even better than alternatives. Then,
make a convincing claim that your simulation structure reflects a meaningful part of
reality.

## What if MaxDiff largely looks the same on other data?

One challenge with using a simulation study to argue against MaxDiff is that
I'm on the hook for justifying my data generating process as representative or
otherwise important. I think the fact my DGP can fairly be described as "literal reproduction of model assumptions" 
combined with a discussion of the psychological weirdness MaxDiff implies here covers importance, but I'm open to the idea that 
your choice data may not look like mine.

Perhaps for example, like above, the topic you study does has a lot more symmetry in
best/worst choicemaking processes than what my simulation implied. Perhaps, like
this [Displayr blog post](https://www.displayr.com/tricked-vs-rank-ordered-logit/), you
also don't find yourself getting meaningfully different answers from models like
mine that are significantly more annoying to implement. Here are a couple of further
thoughts on this type of argument:

First, my suspicion is that ROL with a connected graph of options will outperform
MaxDiff even in the cases where the unknown middle options models give equivalent results to it.
The case for this is pretty straightforward, in the sense that the ROL on connected
graphs model effectively learns a "middle option" from each choice set for ~free. An
interesting extension of the simulation study in this post[^14] would be to generate
both the ROL on connected choices data and preferred MaxDiff data, and see relative
performance given the same underlying simulated respondent characteristics.

[^14]: Exercise left to the reader, but let me know if you do this!

Second, in the real world you don't really get to
know whether your data is the type where MaxDiff largely falls apart. 
Perhaps you have a scenario like the *Displayr* data linked above, but absent
the environment of a simulation study, you don't really get to know which is actually
better. Maybe there are cases like mine where MaxDiff really struggles, and cases
where they're ~equivalent. Again, this is something where I'd suggest making
a compelling case with simulation and explaining how it reflects reality before
treating MaxDiff as a reasonable default tool.
