---
layout: post
title: Better discrete choice modeling through rank ordered logit
...
To estimate this, we recruit and ask them to choose their
...
$$
P(r) = \prod_{j=1}^{J-1} \frac{\exp(U_{ir_j})}{\sum_{k=j}^J \exp(U_{ir_k})}
$$

where $U_{ir_j}$ is the utility of the alternative ranked $j$-th for individual $i$. Here that is in code; the key bit is the `rank_logit_lpmf`
function.
...
# 4. Calculate and print RMSE; couldn't be bothered to find one in a package
...
models I've just shownn are likely the more accurate model for the design you find significant
...
to show that the MaxDiff likelihood isn't a great foundation for extending
discrete choice.
