...
$$
The central terms are the key to the unknown ranking model[^10]. This lets
us fit a ranked model and avoid MaxDiff, even with unknown middle options.

[^10]: Just for fun, notice what happens if there's only 1 middle term. It's your
...
in the `log_sum_exp` tricks I'm using to speed this up, see this footnote[^11].

[^11]: Woo optimization gang! Ok, so the idea here is similar to this block in 
...
exact handling of unknown middle options, and ROL using the Efron approximation
...
# Steelmanning the case for MaxDiff

## What if the errors are actually symmetric?

## What if we only care about population parameters?

## What if you get more information from MaxDiff on larger choice sets?
