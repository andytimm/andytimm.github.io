...
Fortunately, these assignments are nicely scaffolded and provide opinionated input on key design decisions, which reduces the system design burden. So you're asked to draw the rest of the fucking owl[^3],
...
- **Build a Tokenizer:** Tokenization is a primary source of spooky confusing bullshit^TM^ with LLMs[^4]. Building your own tokenizer gives you a finer grained model of how tokenizers work, what tradeoffs exist in tokenization, and (personally) really helped me feel more confident fixing tokenizer issues.
...
much of your recent LLM tinkering, it might be most efficient to spend some hands-on time with a resource like Sebastian Raschka's great [Build a Large Language Model from Scratch](https://www.manning.com/books/build-a-large-language-model-from-scratch).
...
**Leave time to follow your curiosity:** Like Damek Davis noted in his great [live-tweet](https://x.com/damekdavis/status/1968079612044509347) of the course, the number of little research project ideas I had ballooned during this course. I got a ton out of implementing these either myself or via Claude where that felt acceptable. Some things I messed around with:
...
[^1]: Think actually making those GPUs go BRRRR, actually using multiple GPUs well, and understanding what constraints bottleneck you when.

[^2]: Distributed Data Parallel— one of the simpler strategies for training across multiple GPUs by replicating the model on each device and synchronizing gradients.

[^3]: ![](imgs/the_owl.png)

[^4]: I'll cite [SolidGoldMagikarp](https://www.lesswrong.com/posts/aPeJE8bSo6rAFoLqg/solidgoldmagikarp-plus-prompt-generation) for fun. But also, they make [Andrej Karpathy sad](https://www.youtube.com/watch?v=zduSFxRajkE), so you know they're cursed.
