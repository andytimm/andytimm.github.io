...
The $p$-Wasserstein distance between $\xi$ and $p$ is
$$
...
$$
where $\Gamma(\xi, p)$ is the set of couplings between $\xi$ and $p$.
...
amount of dirt/earth in pile $\xi(\theta)$ to make pile $p(\theta)$- hence the name
...
$c(\theta,\theta^{\prime})$. A single transport plan for moving from $\xi(\theta)$ to $p(\theta^{\prime})$ is a function $\gamma(\xi, p)$ which describes the amount of mass to move at each point. If we assume $\gamma$ is a valid joint probability mass with marginals
$\xi\theta)$ and $p(\theta^{\prime})$ [^3], then the infinitesimal mass
...
few posts buy us a better end product; we'll return to trying to fit our MRP
model better.
...
marginalization gives $p(\theta^\prime)$ as you'd expect. Hopefully the
