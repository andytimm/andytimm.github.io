...
$c(theta,theta^{\prime})$. A single transport plan for moving from $\xi(x)$ to $\pi(x)$ is a function $\gamma(\xi, \pi)$ which describes the amount of mass to move at each point. If we assume $\gamma$ is a joint probability mass with marginals
$\xi(x)$ and $\pi(x)$ [^3]
...
moving scenario. Integrating $\gamma$ with respect to $\theta^\prime$ gives
