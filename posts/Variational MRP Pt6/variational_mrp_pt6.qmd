...
![Bayesians love a workflow!](algo.png){width=50%}


This was a very, very long derivation, but hopefully walking through why we would want a distance with a sense of scale and how to calculate it gives you a better feel
for the limitations of our normal divergences used for optimization in variation inference.
...
possible to pinpoint the (a?) source of problems when the bounds are particularly
loose.
...
data does fundamentally change in some way. 
...
the blue distribution will be between the red one and the (black) true posterior. Neat!
...
the paper to be a function of the accuracy you want on the bound, using some
results from Bhatia et al. (2022)- this adaptivity and ability to calculate what number of iterations you need ahead of time is convenient. Second, they're
using a lot of fairly sophisticated techniques in MCMC all together to make this
efficient- strong sampling algorithms like MALA/HMC/HMC, preconditioning, and inter-chain adaptation (INCA) to adapt proposal parameters across the chains
...
variational approximation takes. It's a little opaque to be how exactly that
...
of actual time this takes to run. Still, give it's never several times VI's compute like MCMC would be in
...
for complex posteriors too, then this will be a fantastic new tool. As is,
...
users will lack a sense of magic or wonder that the algorithm otherwise 
