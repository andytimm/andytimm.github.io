...
```
... That looks concerning. What's happened here? The MCMC based estimates
look comparatively unremarkable.
...
1. Whatever's gone wrong has taken what I'd imagine are probably unimodal, smooth
posterior distributions from MCMC off-course to the point where the
Median/50/95% plot no longer seems up to expressing the posterior shape
...
use VI to save a ton of time, infer the intervals consistently need to be wider,
