...
For purposes of this post, I'll
...
samples, but more draws often continues to provide benefits well into the thousands. Keep
...
to the variational parameters. The result is a still unbiased[^5], but lower
...
make this incredibly fast, and comparatively parallel to other variational algorithms. For even moderately complex posteriors, it's blazingly fast, and quite accurate. My only problem with it is that for more challenging posteriors, I've found it a little limited: it doesn't seem to have the representation capacity possible with normalizing flows, which unfortunately is
...
feel off to me. Why is no one releasing a public, serious implementation of this[^7]?
...
Thanks for reading this series, writing it as I learned more and more about
variational inference has been incredibly helpful. I hope to re-visit in the
future and clean up some of the writing and notation. 
...
[^5]: See the paper for a longer explanation for why it's still unbiased, but
...
expectation zero.
[^6]: So many of Bayesian heroes in one place, usually a good
sign.
[^7]: Beyond this [toy example](https://pyro.ai/examples/boosting_bbvi.html) in Pyro,
